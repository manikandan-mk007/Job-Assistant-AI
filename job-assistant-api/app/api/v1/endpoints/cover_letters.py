...
def generate_for_job(
    ...
    if existing:
        ...
        db.commit()
        db.refresh(existing)
        return {"job_id": job.id, "generated_text": existing.generated_text}

    cover_letter = CoverLetter(job_id=job.id, generated_text=generated_text)
    db.add(cover_letter)
    db.commit()
    db.refresh(cover_letter)

    return {"job_id": job.id, "generated_text": cover_letter.generated_text}
